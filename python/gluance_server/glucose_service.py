...
class GlucoseService:
    ...
    async def get_current_reading(self) -> GlucoseReading:
        ...
        raw = await asyncio.wait_for(
            loop.run_in_executor(None, self._client.get_current_glucose_reading),
            timeout=API_TIMEOUT,
        )
    ...
    async def get_readings_history(self, minutes: int = 1440, max_count: int = 288) -> list[GlucoseReading]:
        ...
        raw = await asyncio.wait_for(
            loop.run_in_executor(
                None, partial(self._client.get_glucose_readings, minutes=minutes, max_count=max_count)
            ),
            timeout=API_TIMEOUT,
        )
