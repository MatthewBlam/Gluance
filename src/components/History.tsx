      ...
      <AnimatePresence>
        {!expanded && (
          <motion.div
            ...
            className={twMerge("flex items-center px-4 py-3 gap-1 rounded cursor-pointer select-none", "bg-app-surface hover:bg-app-background-light", className)}>
            {latestValue !== null && (
              <>
                <span className="text-sm text-app-text font-medium">{latestValue}</span>
                <span className="text-sm text-app-text font-medium mr-1.5">{latestReading!.trend_arrow !== "Unavailable" && latestReading!.trend_arrow}</span>
                <RateOfChange />
              </>
            )}
            {latestValue === null && <span className="text-sm text-app-text-muted">No readings</span>}
        ...
        <motion.div
          ...
          <div className="flex gap-1">
            {TIME_RANGES.map((tr) => (
              <button
                ...
                className={twMerge(
                  ...
                  timeRange === tr.minutes ? "bg-brand text-on-brand" : "bg-app-background-light text-app-text-muted hover:text-app-text",
                )}>
                {tr.label}
              </button>
            ))}
            ...
            <button onClick={onToggleExpanded} className="cursor-pointer appearance-none ml-auto relative inline-flex items-center justify-center size-6 rounded bg-transparent">
              <X className="size-5 absolute text-app-text-muted hover:text-app-text transition-all duration-[0.03s]" strokeWidth={2.15} />
            </button>
          </div>
          <div ref={containerRef} className="flex-1 min-h-0 flex flex-col" onPointerMove={onHandlePointerMove} onPointerUp={onHandlePointerUp}>
            ...
            <div onPointerDown={onHandlePointerDown} className="shrink-0 flex items-center justify-center h-3 cursor-row-resize select-none">
              <div className="w-12 h-1 rounded-full bg-app-text-muted/50 hover:bg-app-text-muted transition-colors duration-200" />
            </div>
...
export const HistoryListItem = forwardRef<HTMLDivElement, HistoryListItemProps>(({ children, className, unit, value, mgDl, mmolL, trendDescription, trendArrow, time, date, ...props }, ref) => {
  ...
  const rangeColor = rangeResult === "high" ? "text-reading-high" : rangeResult === "low" ? "text-reading-low" : "text-brand";
  ...
  return (
    <div ref={ref} className={twMerge("flex justify-start px-4 py-3 gap-2.5 content-center align-middle rounded w-full select-none", "bg-app-background-light", className)} {...props}>
      <div className={twMerge("text-sm font-medium", rangeColor)}>•</div>
      <div className="flex gap-1">
        <div className="text-sm text-app-text font-medium">{value === -1 ? "--" : value}</div>

        <div className="text-sm text-app-text font-medium">{value === -1 ? "" : trendArrow}</div>
        <div className="text-sm text-app-text font-medium ml-1.5">{value === -1 ? "" : trendDescription}</div>
      </div>
      <div className="flex gap-2 ml-auto">
        ...
        <div className="text-sm text-app-text font-medium">{value === -1 ? "Unavailable" : formattedDate}</div>
      </div>
    </div>
  );
});
